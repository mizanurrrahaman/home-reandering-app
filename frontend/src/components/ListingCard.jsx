  ...
  const navigate = useNavigate()
   ...
   <div className=" relative cursor-pointer p-2.5 rounded-lg hover:shadow-lg" 
    onClick={()=>{
      ...
      <div className=" w-72 overflow-hidden rounded-lg mb-2.5">
        <div className="flex w-full items-center transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${currentIndex*100}%)`}} >
          {
             listingPhotoPaths?.map((photo, index) =>(
                <div className="relative flex-none w-full h-64 flex items-center" key={index}>
                   <img src={`http://localhost:3000/${photo?.replace("public", " ")}`} alt="" 
                   className="w-full h-full brightness-90"/>

                   <div className="absolute top-1/2 transform -translate-y-1/2 p-1.5 rounded-full border-none cursor-pointer flex items-center justify-center bg-white/70 hover:bg-white left-2.5"
                    onClick={(e)=>{
                      e.stopPropagation()
                      goToPrevSlide(e)
                    }}
                   > 
                   <FaLongArrowAltLeft className="font-[15px]"/>
                   </div>

                   <div className="absolute top-1/2 transform -translate-y-1/2 p-1.5 rounded-full border-none cursor-pointer flex items-center justify-center bg-white/70 hover:bg-white right-2.5"
                    onClick={(e)=>{
                      e.stopPropagation()
                      goToNextSlide(e)
                    }}
                   > 
                   <FaLongArrowAltRight className="font-[15px]"/>
